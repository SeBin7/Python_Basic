...
while count < 5:
    ...
    if count == 4:
        break     # stop at 4
    print("[Python] while count:", count)
...
# if-like behavior: use Python if for scalars, or tf.cond for graph-style branching
t = tf.constant([1, 2, 3, 4, 5])

# boolean mask
tf_mask = (t >= 3) & (t <= 4)
tf_filtered = tf.boolean_mask(t, tf_mask)
print("[TF] mask:", tf_mask.numpy())
print("[TF] filtered:", tf_filtered.numpy())

# iterate tensor (eager mode)
for v in t:
    print("[TF] iter value:", int(v.numpy()))
...
for step, (xb, yb) in enumerate(tf_ds):
    print(f"[TF] step={step}, xb={xb.numpy().tolist()}, yb={yb.numpy().tolist()}")
...
print("[Proj] roi sum:", roi.sum().item())  # number of True positions
...
for epoch in range(20):
    ...
    print(f"[Proj][epoch {epoch}] train={loss.item():.4f} val={val_loss:.4f}")
    ...
    if val_loss < best_val - 1e-4:
        ...
        if no_improve >= patience:
            print("[Proj] early stop triggered")
            break
...
print("[Proj] occluded patch mean:", img[:, :, y:y+h, x:x+w].mean().item())
